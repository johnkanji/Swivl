...
extension LinAlg {
  ...
  public static func gather<T>(_ a: [T], _ indices: [Int]) -> [T] where T:SwivlNumeric {
  ...
  public static func reverseInPlace<T>(_ a: UnsafeMutablePointer<T>, count: Int) where T: SwivlNumeric {
    precondition(memoryCompatible(Double.self, T.self) || memoryCompatible(Float.self, T.self))
  ...
  public static func reverse<T>(_ a: [T]) -> [T] where T: SwivlNumeric {
    var c = a
